...
<div id="page-content" class="page-wrapper clearfix green-mobile-ready green-crm-page">
    <div class="card">
        <div class="page-title clearfix">
            <h1>Anúncios — Conjuntos</h1>
            <div class="title-button-group">
                <?php echo modal_anchor(get_uri("green_crm/ad_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> Novo conjunto", ["class" => "btn btn-primary", "title" => "Novo conjunto", "data-post-type" => "adset"]); ?>
        ...
        <div class="p20">
            ...
            <div class="table-responsive">
                <table id="green-adsets-table" class="display green-ad-table" cellspacing="0" width="100%"></table>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function () {
    if ($.fn.DataTable && $.fn.DataTable.isDataTable("#green-adsets-table")) return;
    function filters() { return {campaign_id: $("#f-campaign").val(), status: $("#f-status").val(), search: $("#f-search").val()}; }
    function apply() {
        var p = filters();
        if (window.InstanceCollection && window.InstanceCollection["green-adsets-table"]) window.InstanceCollection["green-adsets-table"].filterParams = p;
        $("#green-adsets-table").appTable({reload: true, filterParams: p});
    }
    $("#green-adsets-table").appTable({
        source: "<?php echo_uri("green_crm/ad_sets_list_data"); ?>",
        order: [[0, "asc"]],
        tableRefreshButton: true,
        columns: [
            {title: "Conjunto", "class": "all"},
            {title: "Campanha"},
            {title: "ID externo"},
    ...
    $("body").on("click", ".green-ad-delete", function () {
        ...
        $(this).appConfirmation({title: "Excluir registro?", btnConfirmLabel: "Excluir", onConfirm: function () {
            appAjaxRequest({url: "<?php echo_uri("green_crm/delete_ad"); ?>", type: "POST", dataType: "json", data: {id: id, type: type}, success: function (r) {
                ...
                $("#green-adsets-table").appTable({reload: true});
            }});
        }});
    });
