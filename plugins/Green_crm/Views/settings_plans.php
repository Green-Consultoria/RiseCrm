<?php $active_settings_tab = $active_settings_tab ?? "plans"; ?>
    ...
    <div class="card">
        <div class="green-inner-card">
            <div class="page-title clearfix green-crm-page-header">
                ...
                <div class="title-button-group green-crm-title-actions"><?php echo modal_anchor(get_uri("green_crm/plan_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> Novo plano", ["class" => "btn btn-primary", "title" => "Novo plano"]); ?></div>
            </div>
            ...
            <div class="p20 green-crm-content-panel">
                <div class="text-off mb10">Planos cadastrados por operadora, usados em cotacoes e vendas.</div>
                <div class="table-responsive green-table-wrap"><table id="green-plans-table" class="display green-crm-table green-table-settings" cellspacing="0" width="100%"></table></div>
            </div>
        </div>
    </div>
</div>
<script>
$(document).ready(function(){ $("#green-plans-table").appTable({source:"<?php echo_uri("green_crm/plans_list_data"); ?>", tableRefreshButton:true, columns:[{title:"Nome"},{title:"Normalizado"},{title:"Status"},{title:"<i data-feather='menu' class='icon-16'></i>", "class":"all text-center option w120"}], onInitComplete:function(){ if(window.feather){ feather.replace(); } }, onRelaodCallback:function(){ if(window.feather){ feather.replace(); } }}); if(window.feather){ feather.replace(); } });
</script>
